...
public class Sprint1USTest extends SeleniumDriverFactory {
	...
	public void verifyContactUsValidation() {
		...
		try {
			...
			cap.scrollDownAndclickOnSubmitButton();
			Assert.assertEquals("Coty Location Coty Sunnyvale USA has been created.", cap.getCmsStatusMessage(), "auto populated latitude is verified");
			cap.navigateToAdminContent(cotyUrl);
			cap.verifyAddedAddressInList("Coty Sunnyvale USA");
			cap.deleteAddressFromList();
			Assert.assertTrue(cap.getCmsStatusMessage().contains("Coty Location Coty Sunnyvale USA has been deleted."), "Coty address has been deleted");
	...
	public void verifyContactUsImportFunctionality() {
		...
		try {
			int rowCount = dr.getRowCountOfCSVFile();
			launchCotyAdminUrl();
			cap.loginAsAdmin(user, password);
			...
			cap.scrollDownAndclickOnSubmitButton();
			Assert.assertTrue(cap.getCmsStatusMessage().contains("Created"), "Success Message Not Displayed");
			cap.navigateToAdminContent(cotyUrl);	
			cap.selectCheckBox(rowCount);
